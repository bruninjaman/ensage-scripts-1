...
local wispTexture = drawMgr:CreateRect(0,0,imagesize,imagesize,0x000000FF,drawMgr:GetTextureId("NyanUI/modifiers/wisp_relocate_return")) wispTexture.visible = false
...
function Tick(tick)
	...
	if pretime then
		...
		if q then
			wispTime.position = Vector2D(positi.x+distance,positi.y)
			wispTexture.position = Vector2D(positi.x-distance,positi.y)
