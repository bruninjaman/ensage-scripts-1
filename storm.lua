...
local reg = false local victim = nil local attack = 0 local move = 0 local start = false local resettime = nil local movetomouse = nil
...
function Main(tick)
    ...
    local me = entityList:GetMyHero() if me then reg = true end
    if not reg then Close() end
